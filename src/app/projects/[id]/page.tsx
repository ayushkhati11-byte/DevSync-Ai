...
import { ArrowLeft, ExternalLink, Users, MessageSquare, AlertCircle, GitBranch, Send, Check, X, ThumbsUp, Target, BrainCircuit, Trash2 } from "lucide-react";
...
export default function ProjectDetail() {
  ...
  const fetchRequests = useCallback(async () => { try { const r = await fetch(`/api/projects/${id}/requests`); if (r.ok) setPendingRequests(await r.json()); } catch {} }, [id]);
  ...
  useEffect(() => {
    ...
    const loadRequests = async () => {
      try {
        const r = await fetch(`/api/projects/${id}/requests`);
        if (r.ok && !cancelled) setPendingRequests(await r.json());
      } catch {}
    };
  ...
  if (error || !project) return <div className="min-h-screen bg-background"><Navbar /><div className="max-w-3xl mx-auto px-4 py-20 text-center"><AlertCircle className="w-12 h-12 text-white/10 mx-auto mb-4" /><h1 className="text-xl font-bold mb-2">{error || "Not found"}</h1><Link href="/explore" className="text-emerald-400 hover:underline text-sm">Back</Link></div></div>;
  ...
  return (
    <div className="min-h-screen bg-background"><Navbar />
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="bg-white/3 border border-white/6 rounded-xl p-5 sm:p-8 mb-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <a href={project.repoUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-white/6 rounded-lg text-sm hover:bg-white/8 transition-colors"><GitBranch className="w-4 h-4" /> View on GitHub</a>
            {project.deploymentUrl && <a href={project.deploymentUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-white/6 rounded-lg text-sm hover:bg-white/8 transition-colors"><ExternalLink className="w-4 h-4" /> Live Demo</a>}
          ...
          {project.owner && (
            <div className="flex items-center gap-3 pt-5 border-t border-white/6">
              {project.owner.image ? <img src={project.owner.image} alt="" className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 bg-white/6 rounded-full flex items-center justify-center text-sm">{project.owner.name?.[0] || "U"}</div>}
          ...
          {canRequest && requestSent !== "sent" && (
            <div className="mt-5 pt-5 border-t border-white/[0.06]">
              ...
              <textarea value={requestMessage} onChange={(e) => setRequestMessage(e.target.value)} placeholder="Why collaborate? (optional)" className="w-full px-3 py-2 bg-background border border-white/6 rounded-lg text-sm focus:border-emerald-500/50 focus:outline-none text-white/70 placeholder:text-white/20 mb-2" rows={2} maxLength={500} />
          ...
          {canRequest && requestSent === "sent" && <div className="mt-5 pt-5 border-t border-white/6 flex items-center gap-2 text-emerald-400 text-sm"><Check className="w-4 h-4" /> Request sent</div>}
          ...
          {isOwner && pendingRequests.length > 0 && (
            <div className="mt-5 pt-5 border-t border-white/[0.06]">
              ...
              <div className="space-y-2">{pendingRequests.map((req) => (
                <div key={req.id} className="flex items-center justify-between p-3 bg-white/3 rounded-lg border border-white/6">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    {req.user.image ? <img src={req.user.image} alt="" className="w-8 h-8 rounded-full" /> : <div className="w-8 h-8 bg-white/6 rounded-full flex items-center justify-center text-xs">{req.user.name?.[0] || "U"}</div>}
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          <div className="bg-white/3 border border-white/6 rounded-xl p-5 sm:p-6">
            <div className="flex items-center gap-2 mb-4"><BrainCircuit className="w-5 h-5 text-emerald-400" /><h2 className="font-semibold">AI Analysis</h2><div className="ml-auto w-8 h-8 opacity-60"><LottieAnimation src="/animations/ai-processing.json" className="w-full h-full" /></div></div>
            ...
            {project.summary && <p className="text-sm text-white/60 leading-relaxed mt-4 pt-4 border-t border-white/6">{project.summary}</p>}
          </div>
          ...
          {project.members && project.members.length > 0 && (
            <div className="bg-white/3 border border-white/6 rounded-xl p-5 sm:p-6">
              ...
              <div className="space-y-3">{project.members.map((m) => (
                <div key={m.id} className="flex items-center gap-3">{m.user.image ? <img src={m.user.image} alt="" className="w-8 h-8 rounded-full" /> : <div className="w-8 h-8 bg-white/6 rounded-full flex items-center justify-center text-xs">{m.user.name?.[0] || "U"}</div>}
        ...
        {discussions.length > 0 && (
          <div className="bg-white/3 border border-white/6 rounded-xl p-5 sm:p-6">
            ...
            <div className="space-y-3">{discussions.map((d) => (
              <Link key={d.id} href={`/forum/${d.id}`} className="flex items-center justify-between p-3 bg-white/6 rounded-lg hover:bg-white/8 transition-colors">
                <span className="font-medium text-sm">{d.title}</span><span className="text-xs text-white/40">{d._count?.comments || 0} comments</span>
              </Link>
            ))}</div>
          </div>
        )}
